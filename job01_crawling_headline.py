...
# resp = requests.get(url, headers=headers)     ##requests - 웹에서 받는 객체
# # print(list(resp))
# print(type(resp))
...
for i in range(6):
    ...
    titles = []
    for title_tags in title_tags:
        titles.append(re.compile('[^가-힣|a-z|A-Z]').sub(' ', title_tags.text))   ##re.compile('[^가-힣|a-z|A-Z]') --> re_title로 사용가능
